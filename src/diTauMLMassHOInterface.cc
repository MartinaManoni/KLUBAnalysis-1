...
#include "../interface/diTauMLMassHOInterface.h"

namespace ditauMLMassHighOdd {

diTauMLMassHighOdd::diTauMLMassHighOdd(const std::string & model)
{
...
std::vector<float> diTauMLMassHighOdd::GetScore(const float tau1_px, const float tau1_py, const float tau1_pz,const float tau1_e,const float tau2_px,const float tau2_py,const float tau2_pz,const float tau2_e,const float tau1_pt,const float tau1_eta,const float tau1_phi,const float tau2_pt,const float tau2_eta,const float tau2_phi,const float tau1_dm,const float tau2_dm,const float ditau_deltaphi,const float ditau_deltaeta,const float MET_pt, const float MET_phi,const float MET_X,const float  MET_Y,const float DeepMET_ResponseTune_px,const float DeepMET_ResponseTune_py,const float DeepMET_ResolutionTune_px,const float DeepMET_ResolutionTune_py,const float MET_covXX,const float MET_covXY,const float MET_covYY,const int PU_npvs,const float bjet1_pt, const float bjet1_eta,const float bjet1_phi,const float bjet1_deepFlavor,const float bjet1_pNet_b,const float bjet1_pNet_c ,const float bjet1_pNet_uds ,const float bjet2_pt ,const float bjet2_eta ,const float bjet2_phi ,const float bjet2_deepFlavor ,const float bjet2_pNet_b ,const float bjet2_pNet_c ,const float bjet2_pNet_uds ,const float mT1 ,const float mT2 ,const float mTtt ,const float mTtot ,const float mVis , const int N_neutrinos ,const float BDT_channel ,const float BDT_ditau_deltaPhi ,const float BDT_dib_abs_deltaPhi ,const float BDT_dib_deltaPhi ,const float BDT_dau1MET_deltaPhi ,const float BDT_bHMet_deltaPhi ,const float BDT_HT20 ,const float BDT_topPairMasses ,const float BDT_topPairMasses2 , const float BDT_bH_tauH_MET_InvMass ,const float BDT_bH_tauH_InvMass ,const float BDT_total_CalcPhi , const float BDT_ditau_CalcPhi ,const float BDT_dib_CalcPhi ,const float BDT_MET_bH_cosTheta,const float BDT_b1_bH_cosTheta )
{
    tensorflow::Tensor x(tensorflow::DT_FLOAT, tensorflow::TensorShape{1, diTauMLMassHighOdd::n_variables});
...
diTauMLMassHighOdd::~diTauMLMassHighOdd()
{
